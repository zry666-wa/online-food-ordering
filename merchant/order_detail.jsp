...
<div class="col-lg-9">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a href="${ctx}/MerchantServlet?action=orderList" class="btn btn-outline-secondary btn-sm">返回</a>
    </div>
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            ...
            <div class="row small mb-3">
                ...
                <div class="col-md-4">配送地址：${order.address}</div>
            </div>
            ...
            <div class="text-end">
                <h5>合计：<span class="text-danger">¥${order.totalAmount}</span></h5>
            </div>
            <c:if test="${order.status == 1 || order.status == 2}">
                <div class="text-end mt-2">
                    <a href="${ctx}/MerchantServlet?action=orderAdvance&id=${order.id}" class="btn btn-danger btn-confirm" data-confirm="确认推进订单状态？">
                        <c:choose>
                            <c:when test="${order.status == 1}"><i class="bi bi-check2-circle"></i> 接单并开始制作</c:when>
                            <c:when test="${order.status == 2}"><i class="bi bi-bicycle"></i> 出餐并配送</c:when>
                        </c:choose>
                    </a>
                </div>
            </c:if>
